...
void main(void) {
    init_config();
    unsigned char ev[][3]={"ON","GR","GN","G1","G2","G3","G4","C "},key,i = 0,speed = 0,s_flag = 1,l,long_press=0,flag=0,key1;
    ...
    while(1)
    {
        ...
        else if((key == 11 || key == 12) && s_flag == 1)
        {
            clcd_write(0x01, 0);
        ...
        switch(s_flag)
        {
            case 1 : display(ev[i],speed);
                                break;
            case 2 : s_flag = check_pssd(key);
                                break;
            case 3 : s_flag = login_screen(key);
                                if(s_flag == 7)
                                    get_time();
                                break;
            case 4 : s_flag = view_log(key);
                                break;
            case 5 : download_log();
                                s_flag = 1;
                                break;
            case 6 : s_flag = clear_log();
                                break;
            case 7 : clcd_print("  Time    EV  SP",(0x80 + (0)));
                                clcd_print(ev[i],(0xC0 + (10)));
                                clcd_print_ch((speed/10)+'0',(0xC0 + (14)));
                                clcd_print_ch((speed%10)+'0',(0xC0 + (15)));
                                s_flag = set_time(key);
                                break;
            case 8 : s_flag = change_pssd(key);
        }
    }
    return;
}
